...
export default function ProgramPerformancePage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const res = await fetch("/api/dashboard/program-performance");
  ...
  // Calculate overall achieved vs remaining for pie chart
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-6">
      ...
      <Card className="flex-1 shadow-lg rounded-xl">
        <CardHeader>
          <CardTitle className="text-lg font-semibold">Program Performance (Target vs Reported)</CardTitle>
        </CardHeader>
          ...
          <div className="min-w-[800px] h-[400px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={data} margin={{ top: 20, right: 30, left: 20, bottom: 80 }} barGap={10}>
                ...
                <XAxis dataKey="programName" height={70} interval={0} tick={<CustomXAxisTick />} />
                <YAxis
                  label={{
                    value: "People",
                    angle: -90,
                ...
                <Bar dataKey="target" fill="#bc3908" name="Target Unit" radius={[4, 4, 0, 0]} barSize={40} />
